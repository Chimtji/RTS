...
public class NoiseFilterRigid : INoiseFilter
{
    ...
    public NoiseFilterRigid(int size, NoiseSettings settings, Vector2 position)
    {
    ...
    public float Evaluate(Vector3 point)
    {
        ...
        for (int i = 0; i < settings.layers; i++)
        {
            ...
            float v = 1 - Mathf.Abs(noise.Evaluate(pos));
            v *= v;
            v *= weight;
            weight = v;
        ...
        noiseValue = Mathf.Max(0, noiseValue + settings.oceanThreshold);
